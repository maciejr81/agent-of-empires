...
pub(super) const FIELD_HELP: &[FieldHelp] = &[
    ...
    FieldHelp {
        name: "Tool",
        description: "Which AI tool to use (Ctrl+O to configure command and extra args)",
    },
    FieldHelp {
        name: "YOLO Mode",
        description:
    ...
    FieldHelp {
        name: "Worktree",
        description:
            "Branch name for git worktree (Ctrl+O to configure branch mode and extra repos)",
    },
    FieldHelp {
        name: "Sandbox",
        description: "Run session in Docker container for isolation (Ctrl+O to configure)",
    },
    FieldHelp {
        name: "Image",
...
pub struct NewSessionDialog {
    ...
    /// Worktree configuration overlay mode (Ctrl+O on worktree field)
    ...
    /// Tool configuration mode (Ctrl+O on tool field)
...
impl NewSessionDialog {
    ...
    /// Pre-fill dialog fields from an existing session's context
    pub fn with_session_context(
        ...
        self.set_path(session_path.to_string());
        if !session_group.is_empty() {
            self.set_group(session_group.to_string());
        }
    ...
    /// "foo" -> "foo 2", "foo 2" -> "foo 3", skips taken numbers.
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> DialogResult<NewSessionData> {
        ...
        // Worktree sub-options (new_branch, extra_repos) are in a Ctrl+O overlay.
        // Tool config (extra_args, command_override) is in a Ctrl+O overlay on tool field.
        ...
        // Ctrl+O opens a context-sensitive picker/config overlay
    ...
    fn handle_worktree_config_key(&mut self, key: KeyEvent) -> DialogResult<NewSessionData> {
        ...
        match key.code {
            ...
            // Ctrl+O on new_branch field opens branch picker
            KeyCode::Char('p')
    ...
    fn handle_workspace_repos_list_key(&mut self, key: KeyEvent) -> DialogResult<NewSessionData> {
        ...
        if self.workspace_repo_editing_input.is_some() {
            // Ctrl+O: open dir picker for repo path
            if key.code == KeyCode::Char('o') && key.modifiers.contains(KeyModifiers::CONTROL) {
                let initial = self
                    .workspace_repo_editing_input
                    .as_ref()
